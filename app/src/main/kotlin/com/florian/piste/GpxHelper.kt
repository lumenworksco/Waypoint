                ...
                when (parser.name) {
                    "wpt" -> {
                        ...
                        waypoints.add(Waypoint(
                            ...
                            notes = desc, timestamp = time, color = validatedColor(color, "#3C3734")
                        ))
                    }
                    "trkpt" -> {
                        inTrkPt = false
                    ...
                    "trk" -> {
                        ...
                        if (trkPts.isNotEmpty()) {
                            tracks.add(Track(
                                ...
                                color = validatedColor(trkColor, "#007AFF")
                            ))
                        }
                    }
...
/** Returns [hex] if it is a valid color string, otherwise returns [fallback]. */
private fun validatedColor(hex: String, fallback: String): String =
    try { AndroidColor.parseColor(hex); hex } catch (_: Exception) { fallback }
