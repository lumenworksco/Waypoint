...
// ── Lift wait time estimation ──────────────────────────────────

/**
 * Estimate average time spent stationary at lift bases by analyzing GPS data.
 *
 * A "lift wait" is a period where speed is below 0.5 m/s for more than 30 seconds,
 * followed by a lift ride (ascending altitude). Returns the average wait time in
 * milliseconds, or null if no lift waits could be detected.
 */
fun estimateLiftWaitTime(points: List<TrackPoint>): Long? {
    ...
    for (i in 1 until points.size) {
        ...
        if (dt <= 0 || dt > 60_000) continue // skip gaps

        val dist = distanceMeters(
            GeoPoint(prev.latitude, prev.longitude),
            GeoPoint(curr.latitude, curr.longitude)
        )
        val speedMs = dist / (dt / 1000.0)

        if (speedMs < 0.5) {
            // Stationary
            if (stationaryStart == null) stationaryStart = prev.timestamp
        } else {
            // Moving again — check if we were stationary long enough
            if (stationaryStart != null) {
                val waitMs = prev.timestamp - stationaryStart
                if (waitMs >= 30_000) {
                    // Check if this is followed by ascending (lift ride)
                    val prevAlt = prev.altitude ?: 0.0
                    val currAlt = curr.altitude ?: 0.0
                    if (prevAlt > 0.0 && currAlt > 0.0 && currAlt > prevAlt) {
                        waits.add(waitMs)
                    }
                }
    ...
    return if (waits.isEmpty()) null else waits.average().toLong()
}
